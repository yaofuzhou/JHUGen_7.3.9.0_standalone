...
! yRnd(17): accept or reject in unweighted events
    ...
    real(8) :: PostFac, LO_Res_Unpol, PreFac
...
!if pp collider
    if(Collider.eq.1)then
      ...
      call setPDFs(eta1,eta2,pdf)
      FluxFac = 1d0/(2d0*EHat**2)
      PreFac = hbarc2XsecUnit * FluxFac * sHatJacobi * PSWgt * PostFac
      LO_Res_Unpol=0d0
      EvalWeighted_VHiggs=0d0
      do i = -6,6
        j = -i
...
!if e+ e- collider
      ...
      PreFac = hbarc2XsecUnit * FluxFac * PSWgt * PostFac
      LO_Res_Unpol=0d0
      EvalWeighted_VHiggs=0d0
...
!if e+ e- collider
      ...
      FluxFac = 1d0/(2d0*ILC_Energy**2)
      PreFac = hbarc2XsecUnit * FluxFac * PSWgt *6d0 !2 for e and mu, 3 for colors of b
      LO_Res_Unpol=0d0
      EvalWeighted_VHiggs=0d0
      id(2)=convertLHE(ElM_)
      id(1)=-id(2)
...
real(8) :: PostFac, LO_Res_Unpol, PreFac, CS_max, sumtot
logical :: applyPSCut,genEVT
real(8) :: cyRnd(4)
real(8) :: inv_mass(9),mass(9,2)
